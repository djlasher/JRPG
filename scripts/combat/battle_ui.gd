...
func _refresh(message:String):
 ...
 for id in GameState.party:
  ...
  else:var s=GameState.party_state[id];party_rows.append("%s  HP %d/%d  MP %d/%d"%[GameState.PARTY_DEFS[id].name,s.hp,s.max_hp,s.mp,s.max_mp])
 info.text="[b]%s[/b]\n%s\n\n%s"%[message,"\n".join(rows),"\n".join(party_rows)]
func _living():return enemies.filter(func(e):return e.current_hp>0)
func _attack():
 if locked:return
 var target=_living()[0];var damage:=CombatMath.physical_damage(GameState.stat("attack"),int(target.defense),randi_range(-2,2));target.current_hp-=damage;GameState.track("defeat_specific",target.id,1 if target.current_hp<=0 else 0);_after_player("Ari strikes %s for %d damage!"%[target.name,damage])
func _skills():
 if locked:return
...
func _after_player(message:String):
 locked=true;message+=_allies_act()
...
func _allies_act()->String:
 ...
 for id in GameState.party:
  ...
  var target=_living()[0];var damage:=CombatMath.physical_damage(int(member.attack),int(target.defense),0);if id=="lyra":damage=CombatMath.skill_damage(int(member.magic),1.35,int(target.defense));target.current_hp-=damage;text+="\n%s acts for %d damage."%[GameState.PARTY_DEFS[id].name,damage];if target.current_hp<=0:GameState.track("defeat_specific",target.id);GameState.discover_enemy(target.id,true)
 _refresh(text.strip_edges() if text!="" else "Ari completes the action.")
 return text
func _enemy_turn():
 var total=0
 var living_party=[]
 for id in GameState.party:
  var member_hp=GameState.hp if id=="ari" else int(GameState.party_state[id].hp)
  if member_hp>0:living_party.append(id)
 for index in _living().size():
  var e=_living()[index]
  if living_party.is_empty():break
  var target_id:String=living_party[index%living_party.size()]
  var target_defense=GameState.stat("defense") if target_id=="ari" else int(GameState.party_state[target_id].defense)
  var power:int=int(e.attack)+(4 if e.ai in ["aggressive","boss"] else 0)
  var damage:=CombatMath.physical_damage(power,target_defense,randi_range(-1,3))
  if defending:damage=maxi(1,damage/2)
  ...
  total+=damage
 defending=false
 var survivors=GameState.hp
 for id in GameState.party:if id!="ari":survivors+=int(GameState.party_state[id].hp)
