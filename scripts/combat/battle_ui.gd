...
func _build():
 ...
 for enemy in enemies:var portrait=TextureRect.new();portrait.texture=VisualAssets.icon_for_enemy(enemy.id);portrait.custom_minimum_size=Vector2(125,160);portrait.expand_mode=TextureRect.EXPAND_IGNORE_SIZE;portrait.stretch_mode=TextureRect.STRETCH_KEEP_ASPECT_CENTERED;portrait.texture_filter=CanvasItem.TEXTURE_FILTER_LINEAR;portrait.tooltip_text=enemy.name;portrait_row.add_child(portrait);portraits.append(portrait)
