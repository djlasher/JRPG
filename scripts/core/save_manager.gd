...
func load_game()->bool:
 ...
 if not parsed is Dictionary or int(parsed.get("version",0))<1 or int(parsed.get("version",0))>5: last_error="The save is unreadable or incompatible."; return false
