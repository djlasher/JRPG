...
static func background_key_material()->ShaderMaterial:
 var shader=Shader.new();shader.code="shader_type canvas_item; void fragment(){ vec4 c=texture(TEXTURE,UV); float spread=max(c.r,max(c.g,c.b))-min(c.r,min(c.g,c.b)); if(c.r>.72 && c.g>.72 && c.b>.72 && spread<.07){c.a=0.0;} COLOR=c; }"
