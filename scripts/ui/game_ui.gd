...
func shop(id:String,name:String):
 ...
 var stock=["sunleaf_tonic","clearwater_salt","lantern_charm"] if id=="general" else ["reed_sword","ash_staff","wayfarer_vest"]
...
func _rest():
 if GameState.crowns>=24: GameState.crowns-=24; body.text="The lamps dim. You wake rested beneath a quilt of blue herons."; for c in buttons.get_children(): c.queue_free()
 else: body.text="Maeve smiles gently. 'Come back when the road has been kinder.'"
func pause_menu():
 _base("Travel Journal",Vector2(520,320),Vector2(60,20)); mode="pause"; get_tree().paused=true; var mins=int(GameState.play_seconds)/60; body.text="[b]%s[/b]\nLocation: %s\nCrowns: %d\nJourney: %02d:%02d\n\n[b]Items[/b]\n%s"%[GameState.HERO_NAME,GameState.current_location,GameState.crowns,mins,int(GameState.play_seconds)%60,_inventory_text()]
 for option in ["Return to game","Quit to title"]:
  var b=Button.new(); b.text=option; buttons.add_child(b)
 buttons.get_child(0).pressed.connect(_close); buttons.get_child(1).pressed.connect(func(): get_tree().paused=false; clear(); quit_to_title.emit()); buttons.get_child(0).grab_focus()
